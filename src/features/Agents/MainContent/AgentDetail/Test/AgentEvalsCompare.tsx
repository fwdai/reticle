...
function parseTask(inputsJson: string): string {
  try { return JSON.parse(inputsJson).task ?? ""; } catch { return ""; }
}

function parseAgentConfig(snapshotJson: string | null | undefined): string {
  try {
    const s = JSON.parse(snapshotJson ?? "{}");
    return [s.provider, s.model, s.name].filter(Boolean).join(" · ");
  } catch { return ""; }
}
...
export function AgentEvalsCompare({ agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    setLoadingRuns(true);
    listEvalRuns(agentId, "agent").then((rows) => {
      setRuns(rows);
      if (rows.length >= 2) {
  ...
  const configA = useMemo(() => parseAgentConfig(runA?.snapshot_json), [runA]);
  const configB = useMemo(() => parseAgentConfig(runB?.snapshot_json), [runB]);
  ...
  const diff = useMemo(() => {
    ...
    const rows = resultsB.map((bResult) => {
      ...
      if (aResult) {
        ...
        return {
          key: bResult.id!,
          task: parseTask(bResult.inputs_json),
    ...
    const removed = resultsA
      ...
      .map((aResult) => ({
        ...
        task: parseTask(aResult.inputs_json),
            ...
            return (
              ...
              >
                  ...
                  <div className="flex-1 h-1 rounded-full bg-slate-200 overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        background:
                          passed === total ? "hsl(var(--success))"
                          : passed >= total * 0.7 ? "var(--primary)"
                          : "hsl(var(--destructive))",
                      }}
                    />
                  </div>
                ...
                <span className="text-[10px] text-text-muted/60 truncate leading-tight">
                  {parseAgentConfig(run.snapshot_json).slice(0, 60)}
                </span>
              </button>
            );
          })}
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
        {runA && runB ? (
          <>
            ...
            <div className="flex items-center gap-4 border-b border-border-light bg-white px-5 py-3">
              ...
              {improvements > 0 && (
                <span className="flex items-center gap-1 text-xs font-semibold text-green-600">
                  <ArrowUp className="h-3 w-3" /> {improvements} now passing
                </span>
              )}
              {regressions > 0 && (
                <span className="flex items-center gap-1 text-xs font-semibold text-destructive">
                  <ArrowDown className="h-3 w-3" /> {regressions} regression{regressions > 1 ? "s" : ""}
                </span>
              )}
                ...
                {(
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      filterStatus === key ? "bg-primary/15 text-primary shadow-sm" : "text-text-muted hover:text-text-main"
                    )}
                  >
                    {label}
            ...
            {/* Agent config diff collapsible */}
            <button
              onClick={() => setConfigDiffOpen(!configDiffOpen)}
            ...
            >
              {configDiffOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            ...
            {configDiffOpen && (
              <div className="border-b border-border-light bg-white">
                <div className="grid grid-cols-2 divide-x divide-border-light">
                  <div className="p-4">
                    <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-text-muted">Before</span>
                    <p className="text-xs leading-relaxed text-text-muted font-mono whitespace-pre-wrap">{configA}</p>
                  </div>
                  <div className="p-4">
                    <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-text-muted">After</span>
                    <p className="text-xs leading-relaxed text-text-main font-mono whitespace-pre-wrap">{configB}</p>
                  </div>
                </div>
              </div>
            )}
...
function DiffRow({
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: "36px 1fr 1fr 1fr 80px", borderLeftColor: borderColor }}
    >
      <div className="flex items-center justify-center px-2 py-2.5">{statusIcon}</div>

      <div className="px-4 py-2.5">
        <span className="text-xs text-text-muted truncate block">{task}</span>
      </div>
      ...
      <div className="px-4 py-2.5">
        ...
        {Math.abs(latencyDelta) > 0.05 && (
          <span className={cn(
            ...
            {latencyDelta > 0 ? "+" : ""}{latencyDelta.toFixed(2)}
          </span>
        )}
      </div>
    </div>
